...
class HostConfig(BaseModel):
    ...
    overridden by the AIMONT_HOST_ID environment variable.
    ...
    def resolve_id(self) -> str:
        env = os.environ.get("AIMONT_HOST_ID")
        if env:
            return env
        if self.id:
            return self.id
    ...
    def resolve_display_name(self) -> str | None:
        env = os.environ.get("AIMONT_HOST_DISPLAY_NAME")
        if env:
            return env
...
def _apply_push_env_overrides(merged: dict[str, Any]) -> None:
    ...
    1. AIMONT_UPSTREAM_URL (+ AIMONT_TOKEN as plain Bearer).
       The "power-user" path: URL and bearer are separate strings.
    2. AIMONT_TOKEN containing an encoded AimontToken bundle.
...
def _resolve_push_credentials() -> tuple[str | None, str | None]:
    ...
    explicit_url = os.environ.get("AIMONT_UPSTREAM_URL")
    explicit_token = os.environ.get("AIMONT_TOKEN")
    if explicit_url:
        return explicit_url, explicit_token

    # Level 2: AIMONT_TOKEN alone, treated as an encoded bundle.
    if explicit_token:
    ...
    # Level 3: ~/.config/aimont/token file.
    ...
    if file_token:
        ...
        if bundle is not None:
            return bundle.upstream_url, bundle.auth_secret
        logger.warning(
            "Ignoring malformed token at %s; run `aimont leave` to remove it.",
            TOKEN_FILE_PATH,
        )
...
def _try_decode_token(raw: str) -> AimontToken | None:
    try:
