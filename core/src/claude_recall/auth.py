...
A RecallToken bundles everything a daemon needs to connect to an
...
class TokenDecodeError(ValueError):
    """Raised when an input string cannot be decoded into a RecallToken."""


def encode_token(token: RecallToken) -> str:
    """Encode a RecallToken as a compact base64url string.
...
def decode_token(encoded: str, *, verify_key: Any = None) -> RecallToken:
    """Decode a base64url-encoded RecallToken.
    ...
    try:
        return RecallToken.model_validate(payload)
